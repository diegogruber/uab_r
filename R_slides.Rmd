...
- But many applications require more cmplex execution flows:
...
sumofpowers <- function(x = 1:10, power = 2, ...){
  out <- sum(x^power, ...)
  return(out)
}
...
- `eval` evaluates in the moment when the funciton can correctly interpret its value.
...
- columns are explicitly delimited --- tipically with ";" or "," or a tabulator (coded by `"\t"`). 
...
readxl has no external dependencies, it's easy to install and use on all OSs:
